...
export function detectConflicts(candidate: CandidateEntry): ConflictFlag[] {
  ...
  if (occurrences.length > 0) {
    ...
    if (blockedDates.length > 0) {
      ...
      const descriptions = [...new Set(matchedEvents.map(
      ...
      conflicts.push({
        ...
        severity: 'SOFT',
        message: `${blockedDates.length} occurrence(s) cancelled due to ${descriptions.join(', ')}.`,
...
function findTimeOverlaps(
  ...
  // Query existing entries with the same resource, scoped to same academic year + semester
  ...
  const existing = db
    .prepare(
      `SELECT id, start_time, end_time, recurrence_pattern, recurrence_start_date,
       ...
       WHERE ${column} = ? AND is_active = 1 AND id != ? AND academic_year_id = ?${semCondition}`
    )
    .all(value, candidate.id ?? '', candidate.academic_year_id, ...semParams) as ScheduleEntry[]
...
function findSectionTimeOverlaps(
  ...
  // Scope to same academic year + semester to prevent cross-year/semester false positives
  const semCondition = candidate.semester_id ? ' AND semester_id = ?' : ''
  const semParams = candidate.semester_id ? [candidate.semester_id] : []
  const existing = db
    .prepare(
      `SELECT id, section_ids, start_time, end_time, recurrence_pattern, recurrence_start_date,
       recurrence_end_date, day_of_week, day_of_month, week_of_month, custom_days
       FROM schedule_entries
       WHERE is_active = 1 AND id != ? AND section_ids LIKE ? AND academic_year_id = ?${semCondition}`
    )
    .all(candidate.id ?? '', `%${sectionId}%`, candidate.academic_year_id, ...semParams) as ScheduleEntry[]
...
function checkWeeklyOverload(
  ...
  for (const occ of occurrences.slice(0, 10)) { // Check first 10 weeks
    ...
    const existingHours = getPersonnelWeeklyHours(db, personnel.id, weekStart, weekEnd, candidate.academic_year_id, candidate.id, candidate.semester_id)
...
function checkWorkloadApproaching(
  ...
  const weekStart = getWeekStart(occ.date)
  const weekEnd = getWeekEnd(occ.date)
  const existingHours = getPersonnelWeeklyHours(db, personnelId, weekStart, weekEnd, candidate.academic_year_id, candidate.id, candidate.semester_id)
  const candidateHours = calculateHours(candidate.start_time, candidate.end_time)
  ...
  const entries = db
    .prepare(
      `SELECT start_time, end_time, recurrence_pattern, recurrence_start_date, recurrence_end_date,
       ...
       FROM schedule_entries
       WHERE personnel_id = ? AND is_active = 1 AND id != ? AND academic_year_id = ?${semCondition}`
    )
    .all(personnelId, excludeId ?? '', academicYearId, ...semParams) as ScheduleEntry[]

...
import type { GradeLevel, SemesterType } from '../../../shared/types'
...
export function registerSemesterHandlers(): void {
  ...
  registerHandler(IPC_CHANNELS.SEMESTERS_GENERATE_PREVIEW, (args) => {
    const { academic_year_id, grade_level } = args as {
      academic_year_id: string; grade_level: GradeLevel
    }
    const ay = getAcademicYear(academic_year_id)
    return previewSemesterGeneration(ay, grade_level)
  })

  // Execute: save with admin-provided dates (transactional)
  registerHandler(IPC_CHANNELS.SEMESTERS_GENERATE_EXECUTE, (args) => {
    const { academic_year_id, grade_level, semesters } = args as {
      academic_year_id: string; grade_level: GradeLevel
      semesters: Array<{
    ...
    const ay = getAcademicYear(academic_year_id)
    return executeSemesterGeneration(ay, grade_level, semesters)
  })
}
